...
-- ============================================================================
-- This script works with the ACTUAL table structure in your database
...
-- It uses the ACTUAL pickups table structure
...
SELECT 
  ...
  COALESCE(SUM(pk.weight_kg), 0) as total_weight,
  COALESCE(SUM(pk.weight_kg * 18.55), 0) as total_money_value, -- Using average rate
  ...
  'Mixed Materials' as primary_material_type, -- Default since we don't have material breakdown
  18.55 as average_rate_per_kg -- Default average rate
FROM profiles p
...
BEGIN
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    -- Get pickup weight from the ACTUAL pickups table structure
    v_total_weight := COALESCE(NEW.weight_kg, 0);
    
    -- Calculate pickup total value (using average rate)
    v_pickup_total := v_total_weight * 18.55; -- Average rate per kg
    ...
    IF v_wallet_id IS NULL THEN
      ...
      VALUES (NEW.user_id, v_wallet_amount, v_new_total_points, 'Bronze Recycler', NOW(), NOW())
