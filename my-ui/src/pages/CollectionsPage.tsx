...
interface CollectionCardProps {
  collection: { date: string; time: string; area: string; available: boolean };
  bookCollection: (date: string) => void;
}

function CollectionCard({ collection, bookCollection }: CollectionCardProps) {
  const isAvailable = collection.available && new Date(collection.date) >= new Date();
  const isPast = new Date(collection.date) < new Date();
  return (
    <Card className={`shadow-card${!isAvailable ? ' opacity-60' : ''}`}>
...
const CollectionsPage = () => {
  ...
  <Header title="Collections" back />

        <Card className="bg-gradient-primary text-primary-foreground shadow-warm border-0">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Calendar className="h-5 w-5" />
              Next Collection: {nextDate !== 'None scheduled' ? (new Date(nextDate).toLocaleDateString()) : nextDate}
            </CardTitle>
          </CardHeader>
          <CardContent>
