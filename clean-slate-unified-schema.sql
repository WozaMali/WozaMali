-- ============================================================================
-- CLEAN SLATE: DELETE ALL TABLES AND CREATE UNIFIED SCHEMA
-- ============================================================================
-- This script completely wipes the database and creates a clean unified schema
...
BEGIN
    ...
    LOOP
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not drop sequence %: %', sequence_name, SQLERRM;
        END;
    END LOOP;
END $$;
...
BEGIN
    ...
    LOOP
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Could not drop function %: %', func_name, SQLERRM;
        END;
    END LOOP;
END $$;
...
-- ============================================================================
-- 4. CREATE UNIFIED SCHEMA FROM SCRATCH
-- ============================================================================
...
-- Function to calculate points for pickup items
CREATE OR REPLACE FUNCTION calculate_pickup_item_points()
RETURNS TRIGGER AS $$
DECLARE
    material_points INTEGER;
BEGIN
    -- Get points per unit from materials table
    SELECT points_per_unit INTO material_points
    FROM public.materials 
    WHERE id = NEW.material_id;
    
    -- Calculate points earned
    NEW.points_earned = NEW.quantity * COALESCE(material_points, 0);
    
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Trigger to calculate points when pickup item is inserted or updated
CREATE TRIGGER trigger_calculate_pickup_points
    BEFORE INSERT OR UPDATE ON public.pickup_items
    FOR EACH ROW
    EXECUTE FUNCTION calculate_pickup_item_points();
...
UNION ALL
SELECT 
    'Access Test - Material Categories' as table_name,
    COUNT(*) as record_count
FROM public.material_categories
UNION ALL
SELECT 
    'Access Test - Collection Zones' as table_name,
    COUNT(*) as record_count
FROM public.collection_zones;
...
SELECT 
    '🎉 CLEAN SLATE UNIFIED SCHEMA CREATED SUCCESSFULLY' as status,
    'All old tables have been removed and new unified schema is ready' as result,
    'Your database is now clean and ready for all three apps' as next_step,
    'Test your apps to verify everything is working' as note;
