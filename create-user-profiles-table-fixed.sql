-- Create user_profiles table and complete schema (FIXED VERSION)
...
-- 2. Check existing materials table structure
SELECT 
  column_name,
  data_type,
  is_nullable
...
-- 3. Create user_profiles table (only if it doesn't exist)
...
-- 4. Create addresses table (only if it doesn't exist)
...
-- 5. Create wallets table (only if it doesn't exist)
...
-- 6. Create materials table (only if it doesn't exist)
...
-- 7. Create pickups table (only if it doesn't exist)
...
-- 8. Create pickup_items table (only if it doesn't exist)
...
-- 9. Create pickup_photos table (only if it doesn't exist)
...
-- 10. Create payments table (only if it doesn't exist)
...
-- 11. Create rewards table (only if it doesn't exist)
...
-- 12. Create withdrawals table (only if it doesn't exist)
...
-- 13. Create green_scholar_fund table (only if it doesn't exist)
...
-- 14. Insert sample materials (only if table is empty)
INSERT INTO public.materials (name, description, rate_per_kg)
SELECT * FROM (VALUES
...
) AS v(name, description, rate_per_kg)
...
-- 16. Enable Row Level Security (but don't create policies yet)
