...
async function testSoftDelete() {
  ...
  try {
    ...
    // 6. Test active collections view
    const { data: activeViewTest, error: activeViewError } = await supabase
      .from('v_active_unified_collections')
      .select('id')
      .limit(1);
    
    if (activeViewError) {
      console.error('❌ v_active_unified_collections view not found:', activeViewError.message);
      return;
    }
    console.log('✅ v_active_unified_collections view exists');
