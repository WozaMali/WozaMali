...
CREATE TABLE public.collection_pickups (
    ...
    zone_id UUID REFERENCES public.collection_zones(id) ON DELETE CASCADE,
    collector_id UUID REFERENCES public.user_profiles(id) ON DELETE CASCADE,
    customer_id UUID REFERENCES public.user_profiles(id) ON DELETE CASCADE, -- Link to customer profile
    pickup_address_id UUID REFERENCES public.user_addresses(id) ON DELETE SET NULL, -- Link to specific address
    customer_name TEXT NOT NULL, -- Denormalized for quick access
    customer_phone TEXT, -- Denormalized for quick access
    customer_address TEXT NOT NULL, -- Denormalized for quick access (fallback)
    customer_coordinates POINT, -- Denormalized for quick access
...
--   ✅ Multiple addresses per user (primary, secondary, pickup, billing)
...
--   ✅ Adjust user points
--   ✅ View user summaries (including addresses)
--   ✅ Get user addresses
