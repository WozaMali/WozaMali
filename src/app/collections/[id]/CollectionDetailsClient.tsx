      ...
      try {
        ...
        try {
          const { data: c } = await supabase
            .from("collections")
            .select("id, total_value, total_weight_kg, status, created_at, updated_at")
            .eq("id", id)
            .maybeSingle();
        ...
        if (legacy) {
          setData({
            ...
            kgs: Number(legacy.total_weight_kg || 0),
            amount: Number(legacy.total_value || 0),
        ...
        <CardContent className="space-y-2">
          ...
          {!loading && !error && data && (
            <div className="space-y-2">
              ...
              <div className="text-xs text-muted-foreground">
                <span>Created:</span> {data.created_at ? new Date(data.created_at).toLocaleString() : "Unknown"}
              </div>
              <div className="text-xs text-muted-foreground">
                <span>Updated:</span> {data.updated_at ? new Date(data.updated_at).toLocaleString() : "Unknown"}
              </div>
            </div>
          )}
        </CardContent>
